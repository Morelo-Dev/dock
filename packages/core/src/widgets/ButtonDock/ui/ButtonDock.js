...
export function ButtonDock({ children, showMode = false, zIndex, orientation = 'horizontal' }) {
    ...
    const handleDragEnd = useCallback((viewportPos, mode) => {
        commit(mode === 'fixed' ? viewportPos : { x: viewportPos.x + window.scrollX, y: viewportPos.y + window.scrollY }, mode);
    }, [commit]);
    const { onPointerDown } = useDrag({ rootRef, placeholderRef, onDragStart: handleDragStart, onDragEnd: handleDragEnd, onReturnToDock: returnToDock, onSnapChange: setIsNearSnap });
