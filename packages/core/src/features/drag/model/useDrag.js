...
function restoreStyle(el, saved) {
    const set = (prop, val) => val ? el.style.setProperty(prop, val) : el.style.removeProperty(prop);
...
export function useDrag({ rootRef, placeholderRef, onDragStart, onDragEnd, onReturnToDock, onSnapChange, }) {
    ...
    const handleMove = useRef((e) => {
        ...
        if (!isDragging.current)
            return;
        // Use dimensions captured at pointerdown — reliable even after portal switch
        ...
        if (placeholderRef?.current) {
            ...
            const near = e.clientX >= pr.left - SNAP_PADDING_PX &&
    ...
    const handleUp = useRef((e) => {
        ...
        if (placeholderRef?.current) {
            ...
            const overHome = rawX + dockW / 2 >= pr.left - SNAP_PADDING_PX &&
                rawX + dockW / 2 <= pr.right + SNAP_PADDING_PX &&
                rawY + dockH / 2 >= pr.top - SNAP_PADDING_PX &&
                rawY + dockH / 2 <= pr.bottom + SNAP_PADDING_PX;
            if (overHome) {
                onReturnRef.current();
                return;
            }
        }
        // Clamp drop position using stable dimensions captured at pointerdown
        const { x: vx, y: vy } = clamp(e.clientX - offsetX, e.clientY - offsetY, dockW, dockH);
        const nearEdge = vx < 48 || vy < 48 ||
            vx > window.innerWidth - 220 ||
            vy > window.innerHeight - 80;
    ...
    const onPointerDown = useCallback((e) => {
        ...
        drag.current = {
            ...
            // Capture dimensions NOW — before the portal switch that follows startDrag()
            dockW: el.offsetWidth,
